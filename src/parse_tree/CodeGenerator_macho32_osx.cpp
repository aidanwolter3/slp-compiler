...
void CodeGenerator_macho32_osx::write_exit() {
  len += sprintf(code+len, "add esp,%d      ; move stack past local vars\n"
                           "\n;exit\n"
                           "push dword 0\n"
                           "mov eax,1\n"
                           "sub esp,12\n"
...
void* CodeGenerator_macho32_osx::visit(AssignStatement *stm) {
  ...
  if(c2->type == EXPRESSION_STR_PROD) {
    ...
    for(int i = 0; i < strlen(str); i++) {
      len += sprintf(code+len, "mov dword [ebp-%d],%d ; str assign\n", (sym->loc)-(i*4), str[i]);
    }
  }
  else {
...
void* CodeGenerator_macho32_osx::visit(PrintStatement *stm) {
  ...
  len += sprintf(code+len, "lea edx,%s      ; huck\n"
                           "mov eax,4\n"
                           "push dword %d\n"
                           "push dword edx\n"
                           "push dword 1\n"
...
void* CodeGenerator_macho32_osx::visit(StrExpression *exp) {
